...
def build_index(root: Path, data_dir: Path) -> int:
    ...
    favicon_source = Path(__file__).resolve().parent / "assets" / "desktop-visual-gallery.ico"
    (data_dir / "favicon.ico").write_bytes(favicon_source.read_bytes())
...
PAGE = r'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="/favicon.ico" sizes="any"><title>Desktop Visual Gallery</title><style>
